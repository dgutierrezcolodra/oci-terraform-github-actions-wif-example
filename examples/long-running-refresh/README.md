...
OCI provider 8.24.0 owns the OCI UPST and ephemeral RSA key. The local GitHub action refreshes only `OCI_WORKLOAD_IDENTITY_TOKEN_PATH`, using atomic replacement. When the provider needs a new UPST, it rereads the current GitHub JWT and rotates the OCI token and key together.
